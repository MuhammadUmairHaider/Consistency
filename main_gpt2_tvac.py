...
from models.gpt2_fc import GPT2LMHeadModel, GPT2MLP, SplitCProjOnly
...
def replace_mlp_modules(model: nn.Module, config, target_layer: int, current_layer: int = 0):
    for name, module in model.named_children():
        # Check if this module is a Transformer block (common: model.transformer.h.[i])
        if hasattr(module, 'mlp') and isinstance(module.mlp, GPT2MLP):
            if current_layer == target_layer:
                module.mlp = SplitCProjOnly(module.mlp, config)
            current_layer += 1
        else:
            # Recursively traverse deeper
            replace_mlp_modules(module, config, target_layer, current_layer)
            
replace_mlp_modules(model, model.config, layer)
...
probe_all = []
all_fc_vals2 = [None, None]
for i in range(len(all_fc_vals)):
    all_fc_vals2[0] = all_fc_vals[i]
    all_fc_vals2[1] = np.concatenate(all_fc_vals[:i] + all_fc_vals[i+1:], axis=0)
    probe = train_probe(all_fc_vals2).to('cpu')
    probe_all.append(list(probe.parameters())[0][0])
...
for j in range(0,num_classes):
    ...
    mask_probe = compute_mask_probe(probe_all[j], 0.5)
